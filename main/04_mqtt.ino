// Animation types
enum AnimationType {
  ANIM_INSTANT = 0,  // All at once (default)
  ANIM_WAVE,         // Left to right wave
  ANIM_SCROLL,       // Scroll in from right
  ANIM_RANDOM        // Random order reveal
};

// Message queue entry with animation info
struct QueueEntry {
  char text[number_units + 1];
...
bool reconnect() {
  ...
  if (strlen(mqtt_user) > 0) {
    connected = client.connect(mdns_hostname, mqtt_user, mqtt_pass,
                               mqtt_status_topic, 0, true, "offline");
  } else {
    connected = client.connect(mdns_hostname, NULL, NULL,
                               mqtt_status_topic, 0, true, "offline");
  }
